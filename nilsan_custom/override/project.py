...
class project_on_hold(Document):
    ...
    def update_percent_complete(self):
        ...
        if not total:
            ...
            if self.percent_complete_method == "Task Progress" and total > 0:
                progress = frappe.db.sql(
                    """SELECT SUM(progress) FROM `tabTask` 
                    WHERE project=%s""",
                    self.name,
                )[0][0]
            ...
            if self.percent_complete_method == "Task Weight" and total > 0:
                weight_sum = frappe.db.sql(
                    """SELECT SUM(task_weight) FROM `tabTask` 
                    WHERE project=%s""",
                    self.name,
                )[0][0]
                weighted_progress = frappe.db.sql(
                    """SELECT progress, task_weight FROM `tabTask` 
                    WHERE project=%s""",
                    self.name,
                    as_dict=1,
                )
        ...
        # 🚀 FIX: Preserve "On Hold" status and avoid overwriting it with "Open"
